...
LifeLink AI logs every security-sensitive operation to an audit trail that is
independent of the application database. This enables forensic investigation,
compliance reporting, and anomaly detection without risking tampering of the
primary data store.

Logs are written to a local file in JSONL format (one JSON object per line) and
can be shipped to a centralized logging system in production.

The audit module is intentionally Keep-Simple-Embedded (KSE) — no external
dependencies, no background workers, no async I/O. Each audit entry is written
synchronously via a thread‑pool executor so it never blocks request latency.
