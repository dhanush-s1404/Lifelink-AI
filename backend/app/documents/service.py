...
class DocumentService:
    ...
    async def upload(self, upload: UploadInput) -> DocumentUploadOut:
        item = await self._access.require_write_item(
            item_id=upload.vault_item_id, user_id=upload.owner_id
        )
    ...
    async def list_for_item(
        ...
        item = await self._access.require_read_item(
            item_id=vault_item_id, user_id=user_id
        )
        return [
    ...
    async def download(
        ...
        try:
            ...
            raise NotFoundError(
                "Document content is unavailable", code="DOCUMENT_CONTENT_MISSING"
            ) from exc
